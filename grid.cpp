...
Hint::Hint(int r, int c, int cn, char dir) : 
	row(r), col(c), count(cn), direction(dir) {}
...
void Grid::load_from_file(string& file_name){
	...
	for (int i = 0; i < hints_count; ++i) {
		Hint hint;
		char direction;
		file >> hint.row >> hint.col >> hint.value >> direction;
		...
		hint.direction = toupper(direction);
		hints.push_back(hint);
		cells[hint.row][hint.col] = CellState::HINT;
	}
}
...
void Grid::input_from_console() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        while (!(cin >> h.row >> h.col >> h.value >> dir) || 
               h.row < 0 || h.row >= rows || 
               h.col < 0 || h.col >= cols ||
